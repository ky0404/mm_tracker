...
class MMTrackerFactory:
    ...
    def analyze_token(self, symbol: str) -> dict:
        ...
        print(f"\n📊 多时间框架分析:")
        for tf, data in mtf_data.get("layers", {}).items():
            trend = data.get("trend", "unknown")
            rsi = data.get("rsi", 0)
            print(f"  {tf}: trend={trend}, rsi={rsi:.1f}")
...
def main():
    ...
    if args.command == "scan":
        ...
        for i, c in enumerate(results[:10], 1):
            ...
            print(f"{i:2}. {emoji} {symbol:8} 技术:{tech_score:2}/20  24h:{change:+6.1f}%  综合:{combined:.1f}")
        
        print(f"\n✅ 扫描完成! 共 {len(results)} 个候选")
        print("💡 建议: 运行 'run.py analyze <SYMBOL>' 深入分析具体代币")
        ...
        print(f"  python run.py trade                    # 自动交易")
